...
// Dados reais do YouTube importados
...
export function convertYouTubeToPlaylist(youtubeData: YouTubePlaylist[]): Playlist[] {
  return youtubeData.map((item, index) => {
    ...
    const locationMatch = item.title.match(/(CEBB|SP|Rio|BH|Curitiba|Londrina|Paris|Lisboa|Alto Paraíso|Akanishta|Caminho do Meio|Darmata|Sukhavati|Joinville|Floripa|Ponta Grossa)/);
    const location = locationMatch ? locationMatch[1] : 'Brasil';
    
    // Determinar se é destaque baseado no número de itens e recência
    const isFeatured = item.itemCount >= 10 || publishedDate > new Date('2025-01-01');
